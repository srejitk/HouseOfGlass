...
const CartProvider = ({children}) => {
    ...
    const addToCart = async(product) =>{
        if(cartState.cart.some((item)=>item._id===product._id)){
            ...
            try{
                const response = await axios.post("/api/user/cart",{product},header)
    ...
    const deleteFromCart = async(product) =>{
        try{
            const response = await axios.delete(`/api/user/cart/${product._id}`,header)
            if(response.status === 200){
    ...
    const incrementCart = async(product) =>{
        try{
            const response = await axios.post(`/api/user/cart/${product._id}`,{action:{type:"increment"}},header)
            if(response.status === 200){
    ...
    const decrementCart = async(product) =>{
        ...
        try{
            const response = await axios.post(`/api/user/cart/${product._id}`,{action:{type:"decrement"}},header)
            if(response.status === 200){
